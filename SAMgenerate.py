...
device ='cuda'
sam = sam_model_registry ["vit_h"] (checkpoint ="/home/zhangyf/zhangyfcode/SAM_RS/sam_vit_h_4b8939.pth")
...
def SAMAug(tI , mask_generator):
    ...
    if len(masks) == 0:
        ...
        h, w = tI.shape[0], tI.shape[1]
        BoundaryPrior_output = np.zeros((h, w), dtype=np.uint8)  
        Objects_first_few = np.zeros((h, w), dtype=np.uint8)     
...
directory_name='/home/zhangyf/Vai256/img_dir/train/'  
...
for img_input in img_list:
    if img_input.endswith('.png'):
        ...
        image_boundary.save("/home/zhangyf/zhangyfcode/vai_bod_data/"+img_name+'_Boundary'+image_type)
        image_objects.save("/home/zhangyf/zhangyfcode/vai_bod_data/"+img_name+'_objects'+image_type)
end_time = time.time()
