...
def random_scaling(image,obj=None,bod=None,label=None, scale_range=None):
    ...
    return _img_rescaling(image,obj,bod,label, scale=ratio)

def _img_rescaling(image,obj=None,bod=None,label=None, scale=None):
    ...
    new_bod = Image.fromarray(bod).resize(new_scale, resample=Image.NEAREST)  # Use NEAREST for masks/discrete values
    new_bod = np.asarray(new_bod)
 
    new_obj = Image.fromarray(obj).resize(new_scale, resample=Image.NEAREST)  # Use NEAREST for masks/discrete values
    new_obj = np.asarray(new_obj)

    if label is None:
        return new_image,new_obj,new_bod
...
def random_fliplr(image,obj=None,bod=None,label=None):
    p = random.random()
...
def random_crop(image,obj=None, bod=None,label=None, crop_size=None, mean_rgb=[0,0,0], ignore_index=255):
    ...
    if bod is not None:
        pad_bod = np.ones((H, W), dtype=np.uint8) * ignore_index  # or another appropriate fill value
        pad_bod[H_pad:(H_pad + h), W_pad:(W_pad + w)] = bod
        crop_bod = pad_bod[H_start:H_end, W_start:W_end]
 
    if obj is not None:
        pad_obj = np.ones((H, W), dtype=np.uint8) * ignore_index  # or another appropriate fill value
        pad_obj[H_pad:(H_pad + h), W_pad:(W_pad + w)] = obj
        crop_obj = pad_obj[H_start:H_end, W_start:W_end]

    if label is None:
        return crop_image, img_box,crop_obj,crop_bod
