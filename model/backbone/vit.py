...
class VisionTransformer(nn.Module):
    ...
    def forward_featuresyuan(self, x):
    ...
    def forward_features1(self, x):
        ...
        embeds = []
        attns = []
        layer_similarities = []  # 存储每一层的平均token相似度
        ...
        for blk in self.blocks:
            ...
            # 计算当前层所有token之间的余弦相似度
            x_norm = x / (x.norm(dim=-1, keepdim=True) + 1e-8)  # 归一化
            similarity_matrix = torch.matmul(x_norm, x_norm.transpose(-2, -1))  # [B, N, N]
        
            # 计算当前层的平均相似度（排除自相似的对角线）
            mask = ~torch.eye(similarity_matrix.size(1), dtype=torch.bool, device=x.device)
            avg_similarity = similarity_matrix[:, mask].mean().item()  # 标量值
    ...
    # 计算所有层相似度的全局平均值
        global_avg = sum(layer_similarities) / len(layer_similarities)
    
    # 找出最接近全局平均值的层
        closest_layer = min(range(len(layer_similarities)),
                      key=lambda i: abs(layer_similarities[i] - global_avg))
    
    # 动态选择aux_layer（确保不选最后一层）
        aux_layer = min(closest_layer, len(self.blocks) - 2)
    
        return x[:, 0], x[:, 1:], embeds[aux_layer][:, 1:]
    ...
    def forward_features(self, x):
        ...
        # 遍历所有Transformer块，收集每一层的输出
        for blk in self.blocks:
            x, weights = blk(x)
            embeds.append(x)
            attns.append(weights)  # 保留原始注意力权重存储逻辑
        ...
        # -------------------------- 核心优化逻辑 --------------------------
        # 1. 定义要计算的层索引（8、9、10层，注意Python索引从0开始，对应索引7、8、9）
        target_layers = [7, 8, 9]
        
        # 2. 提取这三层的特征（仅取patch部分，即[:, 1:]），并计算特征的均值
        layer_features = []
        for idx in target_layers:
            # 确保索引不越界（鲁棒性处理）
            if 0 <= idx < len(embeds):
                # 取patch特征，并计算该层特征在维度1（token维度）和2（特征维度）上的均值
                feat = embeds[idx][:, 1:]  # shape: [B, N_patch, C]
                feat_mean = torch.mean(feat, dim=(1, 2))  # shape: [B]
                layer_features.append(feat_mean)
            else:
                raise ValueError(f"层索引 {idx} 超出有效范围（有效层数：{len(embeds)}）")
        
        # 3. 计算这三层特征均值的整体平均值（batch维度取平均）
        avg_mean = torch.mean(torch.stack(layer_features, dim=0), dim=0)  # shape: [B]
        avg_mean = torch.mean(avg_mean)  # 缩为标量，便于后续比较
        
        # 4. 计算每一层特征均值与整体平均值的绝对误差，找到最接近的层
        min_error = float('inf')
        selected_layer_idx = None
        for i, idx in enumerate(target_layers):
            error = torch.abs(layer_features[i] - avg_mean).mean().item()  # 计算绝对误差
            if error < min_error:
                min_error = error
                selected_layer_idx = idx
        
        # -------------------------- 输出结果 --------------------------
        # 使用动态选择的中间层，替代原来固定的self.aux_layer
        return x[:, 0], x[:, 1:], embeds[selected_layer_idx][:, 1:]
