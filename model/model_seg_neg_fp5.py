...
class network(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None):
        ...
        self.decoder = decoder.LargeFOV(in_planes=self.in_channels[-1], out_planes=self.num_classes,)   #分割头也为5 特殊
        self.foreground_decoder =  decoder.LargeFOV(in_planes=self.in_channels[-1], out_planes=self.num_classes,)
        self.classifier = nn.Conv2d(in_channels=self.in_channels[-1], out_channels=self.num_classes, kernel_size=1,  #分类头要去除背景
                                    bias=False, )
        self.aux_classifier = nn.Conv2d(in_channels=self.in_channels[-1], out_channels=self.num_classes,  #分类头要去除背景
    ...
    def get_param_groups(self):
        ...
        for param in list(self.foreground_decoder.parameters()):
            param_groups[3].append(param)
    ...
    def similarity(self, patches, context_target, high_k=7):
        ...
        low_order = F.avg_pool2d(patches, kernel_size=high_k, stride=1, padding=(high_k - 1) // 2) - patches / (
                high_k ** 2)
        low_order *= (high_k ** 2) / (high_k ** 2 - 1)
        high_order = (patches.mean(dim=(2, 3), keepdim=True) - patches / (img_size ** 2)) * (img_size ** 2) / (
                img_size ** 2 - 1)
    ...
    def forward(self, x, cam_only=False, crops=None, n_iter=None):
        ...
        foreground_seg = self.foreground_decoder(_x4)
        if cam_only:
        ...
        if crops is None:
            ...
            return cls_x4, seg, foreground_seg, _x4, cls_aux, output_t, output_s
