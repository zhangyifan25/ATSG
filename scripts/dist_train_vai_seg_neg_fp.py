...
from utils.camutils_ori import cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg,dino_mask
...
parser.add_argument("--data_folder", default='/home/zhangyf/Vai256', type=str, help="dataset folder")
parser.add_argument("--list_folder", default='/home/zhangyf/zhangyfcode/ATSG/datasets/vai', type=str, help="train/val/test list file")
...
parser.add_argument("--work_dir", default="work_dir_vai_wseg", type=str, help="work_dir_vai_wseg")
...
parser.add_argument("--max_iters", default=15000, type=int, help="max training iters")
...
parser.add_argument("--w_bod", default=1.2, type=float, help="w_bod")
parser.add_argument("--w_obj", default=0.01, type=float, help="w_obj")
...
def train(args=None):
    ...
    train_dataset = vai.vaiClsDataset(
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            img_name, inputs, cls_label, img_box, crops,obj,bod = next(train_loader_iter)
        ...
        aff_mask = dino_mask(pseudo_label_aux,dino_map)

        ptc_loss = get_masked_ptc_loss(fmap, aff_mask)
  

        objloss=ObjectLoss()
        bodloss=BoundaryLoss()
        obj_loss=objloss(segs,obj)
        bod_loss=bodloss(segs,bod)
        ...
        if n_iter <= 2000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss  + 0.0 * seg_loss + 0.0 * reg_loss + 0.0 * joint_loss + args.w_bod * bod_loss + args.w_obj * obj_loss 
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss  + args.w_seg * seg_loss + args.w_reg * reg_loss + args.w_joi * joint_loss + args.w_bod * bod_loss + args.w_obj * obj_loss 
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, cls_loss_aux: %.4f, ptc_loss: %.4f, ctc_loss: %.4f, seg_loss: %.4f..., joint_loss: %.4f...,bod_loss: %.4f,obj_loss: %.4f," % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('ptc_loss'), avg_meter.pop('ctc_loss'), avg_meter.pop('seg_loss'),avg_meter.pop('joint_loss'),avg_meter.pop('bod_loss'),avg_meter.pop('obj_loss')))
        ...
        if (n_iter + 1) % args.eval_iters == 0:
            ...
            if args.save_ckpt and (n_iter + 1) >= 1000 and mIoU_result[2] > mIoU:
                mIoU = mIoU_result[2]
