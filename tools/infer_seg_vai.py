...
def _validate(model=None, data_loader=None, args=None):
    ...
    print(format_tabs([seg_score], ["seg_pred"], cat_list=vai.class_list))
...
def crf_proc():
    ...
    images_path = os.path.join(args.data_folder, 'img_dir','val')
    labels_path = os.path.join(args.data_folder, 'ann_dir', 'val')
    ...
    def _job(i):
        ...
        imageio.imsave(args.segs_rgb_dir + "/" + name + ".png", imutils.encode_cmap4potsdam(np.squeeze(pred)).astype(np.uint8))
        return pred, label
    ...
    print(format_tabs([crf_score], ["seg_crf"], cat_list=vai.class_list))
    return crf_score
